...
文档和示例默认使用 `eno0`。如果 PC2 实际机器人通信网卡不是 `eno0`，后续命令里的 `net=eno0` 和 `./build/csv_replay ... eno0` 要替换为实际网卡名。
...
ExecStart=/home/unitree/g1_motion_player/.venv/bin/uvicorn api.main:app --host 127.0.0.1 --port 8000
Restart=on-failure
RestartSec=2
